...
def cws(bsdf, emitter, sampler, scene, si):
    ...
    # u1 = sample2.x  # [N]
    # u2 = sample2.y  # [N]
    # theta = dr.acos(dr.power(u1, 1 / (n + 1)))
    # phi = 2 * math.pi * u2
    #
    # wo_local2 = mi.Vector3f(0, 0, 0)
    # wo_local2.x = dr.sin(theta) * dr.cos(phi)
    # wo_local2.y = dr.sin(theta) * dr.sin(phi)
    # wo_local2.z = dr.cos(theta)
    ...
    # theta = dr.acos(dr.power(u1, 1 / (n + 1)))
    phi = 2 * math.pi * u1
    sinAlpha = dr.sqrt(1 - dr.power(u2, 2 / (n + 1)))
    cosAlpha = dr.power(u2, 1 / (n + 1))

    wo_local2 = mi.Vector3f(0, 0, 0)
    wo_local2.x = sinAlpha * dr.cos(phi)
    wo_local2.y = sinAlpha * dr.sin(phi)
    wo_local2.z = cosAlpha

    print(wo_local2.x.torch()*wo_local2.x.torch() + wo_local2.y.torch()*wo_local2.y.torch() + wo_local2.z.torch()*wo_local2.z.torch())

    wi = mi.Vector3f(1,1,1)
    wi = dr.normalize(wi)
    wi_reflect = mi.reflect(wi, mi.Vector3f(0, 0, 1))
    frame = mi.Frame3f(wi_reflect)
    wo_local2 = frame.to_world(wo_local2)


    spp = 128
    x= wo_local2.x.torch()[:spp].cpu()
    y= wo_local2.y.torch()[:spp].cpu()
    z= wo_local2.z.torch()[:spp].cpu()
