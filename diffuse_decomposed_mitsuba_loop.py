...
def render(cam_angle_x, cam_transform_mat, imh, imw, scene, spp, debug):
    ...
    loop = mi.Loop(name='Path Tracer', state=lambda: (sampler, ray, throughput, result,
...
def main():
    ...
    for i in range(n):
        ...
        # show_image(image)
        if not debug:
            save_image_path = f"./tests/diffuse_decomposed/{expeirment}_{n}_mitsuba_loop/{view}.png"
